...
- (AppvisorCustomAlertView * _Nullable)showAlertFor:(UNNotification * _Nonnull)notification customWidth:(CGFloat)customWidth customHeight:(CGFloat)customHeight with:(void (^ _Nonnull)(void))cancelAction and:(void (^ _Nonnull)(void))goAction SWIFT_WARN_UNUSED_RESULT;
...
/// Tracks a push notification that the user tapped/opened.
/// This method should be called from <code>userNotificationCenter(_:didReceive:withCompletionHandler:)</code>
/// when a user taps on a notification. It extracts the push ID from the notification payload,
/// sends tracking data to the Appvisor server, and opens any associated URL if present.
/// seealso:
/// <code>trackSilentPush(with:)</code> for handling silent push notifications.
/// \param userInfo The notification payload from the tapped notification.
///
- (void)trackPushWith:(NSDictionary<NSString *, id> * _Nullable)userInfo;
/// Tracks a silent push notification.
/// Call this method from <code>application(_:didReceiveRemoteNotification:fetchCompletionHandler:)</code>
/// to handle silent push notifications.
/// \param userInfo The notification payload.
///
///
/// returns:
/// <code>UIBackgroundFetchResult</code> indicating the result of processing:
/// <ul>
///   <li>
///     <code>.newData</code>: Successfully processed an Appvisor push notification
///   </li>
///   <li>
///     <code>.noData</code>: Not an Appvisor notification or no push ID found
///   </li>
/// </ul>
- (UIBackgroundFetchResult)trackSilentPushWith:(NSDictionary * _Nullable)userInfo SWIFT_WARN_UNUSED_RESULT;
