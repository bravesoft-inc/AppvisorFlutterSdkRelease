...
public class AppvisorFlutterSdkPlugin: NSObject, FlutterPlugin {
    ...
    private func handleSyncCustomProperties(
        _ call: FlutterMethodCall, _ result: @escaping FlutterResult
    ) {
        log(tag, "Syncing user properties.")
        avp.synchronizeUserProperties { syncResult in
            guard syncResult.isSuccess else {
                let error = syncResult.error
                result(
                    FlutterError(
                        code: error?.code.stringValue ?? AvpError.Unknown.rawValue,
    ...
    private func handleGetNotices(_ call: FlutterMethodCall, _ result: @escaping FlutterResult) {
        ...
        avp.getNotices(with: messageId, and: userUUID, completion: {avresult, response in
            guard avresult.isSuccess else {
                let error = avresult.error
                result(
                    FlutterError(
                        code: error?.code.stringValue ?? AvpError.Unknown.rawValue,
                        message: error?.code.message,
    ...
    private func handleGetConfig(_ result: @escaping FlutterResult) {
        avp.getCurrentConfig(completion: {avresult, config in
            guard avresult.isSuccess else {
                let error = avresult.error
                result(
                    FlutterError(
    ...
    private func handleMarkNoticeAsRead(_ call: FlutterMethodCall, _ result: @escaping FlutterResult) {
        log(tag, "Marking notice as read.")
        ...
        avp.setNoticeRead(with: messageId) { r in
            if r.isSuccess {
                log(tag, "Notice marked as read successfully.")
                result(nil)
            } else {
                let error = r.error
                log(tag, "Failed to mark notice as read: \(error?.message ?? "Unknown error")")
                result(FlutterError(
                    code: error?.code.stringValue ?? AvpError.Unknown.rawValue,
                    message: error?.code.message,
                    details: error?.message
    ...
    public func application(_ application: UIApplication, didRegisterForRemoteNotificationsWithDeviceToken deviceToken: Data) {
        ...
        // UPDATE:
        // As of https://github.com/bravesoft-inc/Appvisor-iOS-SDK/commit/546a9c9f125ee69aaba9c2ca15303250c982bc02,
        // calling enablePush() no longer changes the UNUserNotificationCenter delegate.
        // This code to reassign the delegate remains for compatibility and precaution.
        // If future issues arise, consider removing this reassignment.
        log(tag, "Device token received.\nReassigning the UNUserNotificationCenter delegate to self.")
        UNUserNotificationCenter.current().delegate = self
        avp.registerToken(with: deviceToken, completion: { result in
            if let error = result.error {
                log(tag, "Failed to register device token:")
                log(tag, error.code.message)
            }
        })
    }
    ...
    public func userNotificationCenter(_ center: UNUserNotificationCenter, didReceive response: UNNotificationResponse, withCompletionHandler completionHandler: @escaping () -> Void) {
        ...
        log(tag, "Notification is tapped: \(notifData)")
